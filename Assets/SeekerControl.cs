...
public class SeekerControl : MonoBehaviour
{
    ...
    {
        if (Input.GetKey(KeyCode.UpArrow)) speed += acceleration * Time.deltaTime;
        if (Input.GetKey(KeyCode.DownArrow)) speed -= acceleration * Time.deltaTime;
        if (Input.GetKey(KeyCode.LeftArrow)) direction -= angularSpeed * Time.deltaTime;
        if (Input.GetKey(KeyCode.RightArrow)) direction += angularSpeed * Time.deltaTime;
